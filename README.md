# Документация для сборки и использования в Docker

## Сборка
docker build -t spectryyy/python-calculator:1.0 .
## Запуск (с автоудалением после остановки)
docker run -it --rm --name ipr-calculator spectryyy/python-calculator:1.0
## Стандартный запуск
docker run -it --name ipr-calculator spectryyy/python-calculator:1.0
## Перезапуск после остановки (Ctrl + C)
docker start -i ipr-calculator

# Оригинальная документация для RPN-калькулятора
